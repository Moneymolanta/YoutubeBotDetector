...
for epoch in range(EPOCHS):
    ...
    if test_accuracy > best_accuracy:
        ...
        # Save the best model
        torch.save(model.state_dict(), 'best_bot_detector_model.pth')
        print("Saved best model checkpoint.")

# Load the best model
print(f'Best Accuracy: {best_accuracy:.4f}')
model.load_state_dict(torch.load('best_bot_detector_model.pth'))
...
def predict_bot_comments(model, filepath, dataset_name):
    ...
    print(
        f"Bot confidence - High: {high_confidence_bots}, Medium: {medium_confidence_bots}, Low: {low_confidence_bots}")
